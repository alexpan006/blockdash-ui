...
const logger = getLogger("nakamotobar");

export default function NakamotoBarChart() {
    ...
    try {
      ...
      const overallResponses = await Promise.all(
        relationTypeOptionsOverall.map(async (relationType) => {
          const overallUrl = `/api/nakamotobar/overall?${queryParams.toString()}&relation_type=${relationType}`;
      ...
      const singleTypeResponses = await Promise.all(
        relationTypes.map(async (relationType) => {
          const singleTypeUrl = `/api/nakamotobar/singletype?${queryParams.toString()}&relation_type=${relationType}`;
            ...
            title={
              <Box sx={{ textAlign: "justify" }}>
                <Typography variant="body2">
                  The Nakamoto coefficient is a measure used to assess the
                  decentralization of a blockchain or distributed system. It
                  represents the minimum number of entities required to
                  collectively control more than 50% of a given resource, such
                  as transactions, ownership, or minting power.
                </Typography>
              </Box>
            }
            placement="right"
          ...
          {singleTypeData.length > 0 ? (
            <Box sx={{ width: "100%", maxWidth: isMobile ? "100%" : "100%" }}>
              <BarChart
                ...
                width={isMobile ? 350 : 1350} // Provide a number instead of a string
              />
            </Box>
          ) : (
