...
export async function GET(request: Request) {
  ...
  if (!["transacted", "mint", "owned"].includes(relation_type.toLowerCase())) {
  ...
  try {
    ...
    const apiUrl = `${process.env.API_URL}/equality/nakamoto_history?year_from=${encodeURIComponent(
      year_from
    ...
    if (data.dates && data.counts) {
      ...
      return NextResponse.json(
        { error: "Data is missing expected fields" },
        { status: 500 }
      );
    }
  } catch (error) {
